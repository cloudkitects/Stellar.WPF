...
public abstract class VisualLineElement
{
    ...
    /// <summary>
    /// Creates a new visual line element.
    /// </summary>
    ...
    protected VisualLineElement(int visualLength, int documentLength)
    {
        if (visualLength < 1)
        {
            throw new ArgumentOutOfRangeException(nameof(visualLength), $"{visualLength} < 1");
        }

        if (documentLength < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(documentLength), $"{documentLength} < 0");
        }
    ...
    /// </param>
    public abstract TextRun CreateTextRun(int startVisualColumn, ITextRunContext context);
    ...
    public virtual TextSpan<CultureSpecificCharacterBufferRange> GetPrecedingText(int visualColumnLimit, ITextRunContext context)
    {
        return null!;
    }
    ...
    protected void SplitHelper(VisualLineElement firstPart, VisualLineElement secondPart, int splitVisualColumn, int splitRelativeTextOffset)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(splitVisualColumn), $"Value must be between {VisualColumn + 1} and {VisualColumn + VisualLength - 1}");
        }

        if (relativeSplitRelativeTextOffset < 0 || relativeSplitRelativeTextOffset > DocumentLength)
        {
            throw new ArgumentOutOfRangeException(nameof(splitRelativeTextOffset), $"Value must be between {RelativeTextOffset} and {RelativeTextOffset + DocumentLength}");
        }
    ...
    /// </summary>
    public virtual int GetVisualColumn(int relativeTextOffset) => VisualColumn + (relativeTextOffset >= RelativeTextOffset + DocumentLength ? VisualLength : 0);
    ...
    public virtual int GetRelativeOffset(int visualColumn) => RelativeTextOffset + (visualColumn >= VisualColumn + VisualLength ? DocumentLength : 0);
