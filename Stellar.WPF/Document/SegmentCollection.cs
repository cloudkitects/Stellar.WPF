...
/// <summary>
/// A collection of text segments for fast segment intersection lookup.
/// </summary>
/// <remarks><inheritdoc cref="Segment"/></remarks>
/// <see cref="Segment"/>
public sealed class SegmentCollection<T> : ICollection<T>, ISegmentTree, IWeakEventListener where T : Segment
{
    ...
    #region Constructor
    /// <summary>
    /// Creates a new SegmentCollection that needs manual calls to <see cref="UpdateOffsets(DocumentChangeEventArgs)"/>.
    /// </summary>
    ...
    /// <summary>
    /// Creates a new SegmentCollection that updates the offsets automatically.
    /// </summary>
    /// <param name="Document">The document to which the text segments
    ...
    public SegmentCollection(Document Document)
    {
        if (Document is null)
        {
        ...
        WeakDocumentEventManager.Changed.AddListener(Document, this);
    }
    #endregion
    ...
    {
        if (managerType != typeof(WeakDocumentEventManager.Changed))
        {
            return false;
        }
    ...
    public void UpdateOffsets(DocumentChangeEventArgs e)
    {
        ...
        {
            throw new InvalidOperationException("Only the segment collection can update offsets.");
        }
    ...
    private void OnDocumentChanged(DocumentChangeEventArgs e)
    {
        ...
        {
            UpdateOffsetsInternal(e.CreateChange());
        }
    }
    ...
    private void UpdateOffsetsInternal(ChangeOffset change)
    {
        // pure insertions don't always cause a segment to grow when the replaced region
    ...
    private void InsertText(int offset, int length)
    {
        ...
        foreach (Segment segment in FindSegmentsContaining(offset))
        {
        ...
        Segment node = FindFirstSegmentWithStartAfter(offset);
        if (node != null)
        {
    ...
    private void ReplaceText(ChangeOffset change)
    {
        Debug.Assert(change.RemovalLength > 0);
        int offset = change.Offset;
        foreach (Segment segment in FindOverlappingSegments(offset, change.RemovalLength))
        {
        ...
        // move start offsets of all segments > offset
        Segment node = FindFirstSegmentWithStartAfter(offset + 1);
        if (node != null)
        {
    ...
    void ISegmentTree.Add(Segment s)
    {
        AddSegment(s);
    }

    private void AddSegment(Segment node)
    {
        ...
        else
        {
            ...
            Segment n = FindNode(ref insertionOffset);
    ...
    private void InsertBefore(Segment node, Segment newNode)
    {
        if (node.left == null)
        {
    ...
    /// </summary>
    private Segment FindNode(ref int offset)
    {
        Segment n = root;
        while (true)
        {
    ...
    /// This means it is safe to modify the SegmentCollection while iterating through the result collection.</returns>
    ...
    private void FindOverlappingSegments(List<T> results, Segment node, int low, int high)
    {
    ...
    private void UpdateAugmentedData(Segment node)
    {
    ...
    void ISegmentTree.UpdateAugmentedData(Segment node)
    {
        UpdateAugmentedData(node);
    }
    #endregion
    ...
    void ISegmentTree.Remove(Segment s)
    {
        RemoveSegment(s);
    }

    private void RemoveSegment(Segment s)
    {
        int oldOffset = s.StartOffset;
        Segment successor = s.Successor;
        if (successor != null)
        {
    ...
    private void Disconnect(Segment s, int offset)
    {
...
#if DEBUG
    private void CheckProperties(Segment node)
    {
        int totalLength = node.nodeLength;
        int distanceToMaxEnd = node.segmentLength;
        if (node.left != null)
     ...
     */
    private void CheckNodeProperties(Segment node, Segment parentNode, bool parentColor, int blackCount, ref int expectedBlackCount)
    {
        if (node == null)
        {
            return;
    ...
    private static void AppendTreeToString(Segment node, StringBuilder b, int indent)
    {
        if (node.color == RED)
        {
            b.Append("RED   ");
    ...
    private void InsertAsLeft(Segment parentNode, Segment newNode)
    {
    ...
    private void InsertAsRight(Segment parentNode, Segment newNode)
    {
    ...
    private void FixTreeOnInsert(Segment node)
    {
        ...
        Segment parentNode = node.parent;
        if (parentNode == null)
        {
        ...
        Segment grandparentNode = parentNode.parent;
        Segment uncleNode = Sibling(parentNode);
    ...
    private void RemoveNode(Segment removedNode)
    {
        if (removedNode.left != null && removedNode.right != null)
        {
            // replace removedNode with it's in-order successor

            Segment leftMost = removedNode.right.LeftMost;
        ...
        Segment parentNode = removedNode.parent;
        Segment childNode = removedNode.left ?? removedNode.right;
    ...
    private void FixTreeOnDelete(Segment node, Segment parentNode)
    {
        ...
        Segment sibling = Sibling(node, parentNode);
    ...
    private void ReplaceNode(Segment replacedNode, Segment newNode)
    {
    ...
    private void RotateLeft(Segment p)
    {
    ...
    private void RotateRight(Segment p)
    {
    ...
    private static Segment Sibling(Segment node)
    {
    ...
    private static Segment Sibling(Segment node, Segment parentNode)
    {
        Debug.Assert(node == null || node.parent == parentNode);
        if (node == parentNode.left)
        {
    ...
    private static bool GetColor(Segment node)
    {
